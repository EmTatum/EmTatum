...
# Use .[]? to avoid errors if it is not an array, then select to ensure valid elements
REPO_LIST=$(echo "$RESPONSE" | jq -r 'if type == "array" then .[] | "- [\(.name)](\(.html_url))" else empty end')
